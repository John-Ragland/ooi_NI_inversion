...
if __name__ == "__main__":
    ...
    for threshold in tqdm([3,4,5]):
        ...
        NCCFs601 = NCCFs.rolling(dates=601, min_periods=100).mean()
        ...
        for k in range(3):
            filtered_NCCFs_A.append(xrsignal.filtfilt(NCCFs_avg[k], dim='delay', b=bs_A[k], a=as_A[k]))
            filtered_NCCFs_A[-1].attrs = {'fc':fcs[k][0], 'avg_hours':avg_hours[k]}
            filtered_NCCFs_B.append(xrsignal.filtfilt(NCCFs_avg[k], dim='delay', b=bs_B[k], a=as_B[k]))
